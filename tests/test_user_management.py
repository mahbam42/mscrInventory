...
@pytest.mark.django_db
def test_manage_users_can_create_user(client, view_user_permission):
    manager = get_user_model().objects.create_user("manager", password="pw", is_staff=True)
    manager.user_permissions.add(view_user_permission)
...
@pytest.mark.django_db
def test_manage_users_can_reset_password(client, view_user_permission):
...
@pytest.mark.django_db
def test_navigation_links_include_admin_and_manage_users(view_user_permission):
